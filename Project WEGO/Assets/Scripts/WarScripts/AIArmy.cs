...
public class AIArmy : ArmyManager
{
	...
	public override void SetEnemyArmy(ArmyManager enemy)
	{
		EnemyArmy = enemy;
	...
	public override void SetupTokens()
	{
		...
		for (int i = 0; i < armySize; i++)
		{
			...
			//if (type != "Ranged")
				hexMap.ChangeGoodnessFromTokens(ValidHexes[counter], Army[i].GetUnitTypeInt(), playerID, true);
		}
					...
					foreach (var item in currentHexes)
					{
						...
						foreach (var t in tokens)
						{
							...
							while (counter < thisValidMove.Length)
							{
								...
								{
									hexMap.UpdateGoodness(t.GetCurrentHex(), thisValidMove[counter], t.GetUnitTypeInt(), playerID);
